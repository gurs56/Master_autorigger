...
def main():
    ...
    """
    # Look at all joints 
    LegJoints = cmds.ls(type='joint')
      
    # Target joint names to look for
    target_names = ["JNT_thigh", "JNT_calf", "JNT_foot", "JNT_ball", "JNT_toe"]
            ...
            for target in target_names:
                if target in joint:
                    if joint.endswith("_l"):
                        LLegJoints.append(joint)
                    elif joint.endswith("_r"):
                        RLegJoints.append(joint)

    # Print results for debugging
    print "Left Leg Joints: {}".format(LLegJoints)
    print "Right Leg Joints: {}".format(RLegJoints)
    
    # Sort the joints based on the target hierarchy
    LLegJoints = sort_joints(LLegJoints, target_names)
    RLegJoints = sort_joints(RLegJoints, target_names)

    # Duplicate IK and FK joints for left and right arms
    ik_joints_left = duplicate_joints(LLegJoints, "_IK")
    fk_joints_left = duplicate_joints(LLegJoints, "_FK")
    ik_joints_right = duplicate_joints(RLegJoints, "_IK")
    fk_joints_right = duplicate_joints(RLegJoints, "_FK")
    ...
    # Print final results
    print "Left Leg IK Joints: {}".format(ik_joints_left)
    print "Left Leg FK Joints: {}".format(fk_joints_left)
    print "Right Leg IK Joints: {}".format(ik_joints_right)
    print "Right Leg FK Joints: {}".format(fk_joints_right)
