...
def main():
    ...
    subSpine3 = 'CNT_SUB_SPINE_3_C'
    
    # neck controllers
    
    joints = [i for i in process.get_option('spine', group = 'Rig Bone Groups') if i.find('neck') != -1]
    
    neck = rigs.FkRig('neck', 'C')
    neck.set_joints(joints)
    neck.set_control_offset_axis('z')
    neck.set_control_size(4)
    ...
    neck.delete_setup()
    neck.create()
    neck.set_control_parent( subSpine3 )
